...
function checkval($select, $from, $WHERE = NULL, $AND = NULL,$AND_2= NULL, $value1, $value2=NULL,$value3 = NULL){

  global $con;

  $statement = $con->Prepare("SELECT $select FROM $from $WHERE $AND $AND_2");
  if ($value2 == NULL) {
...
function datediff($select,$date ,$where, $and = NULL){

  global $con;

   $getdate = $con->prepare("SELECT $select , DATEDIFF(CURDATE(),$date) AS Added FROM users $where $and ");
